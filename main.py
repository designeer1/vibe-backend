...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.ui = Ui_vibeGUI()  # Initialize the UI from vibeUI.py
        self.ui.setupUi(self)  # Setup the UI in the main window
        self.ui.pushButton.clicked.connect(self.startTask)
        self.ui.pushButton_2.clicked.connect(self.closeTask)
        self.setWindowTitle("PyQt5 Web Interface")  # Title of the window
        self.setGeometry(100, 100, 800, 600)  # Set window size and position

        # Connect the Run and Terminate buttons to their respective actions
        self.ui.pushButton.clicked.connect(self.startTask)  # Run button
        self.ui.pushButton_2.clicked.connect(self.closeTask)  # Terminate button
...
if __name__ == "__main__":
    app = QApplication(sys.argv)  # Ensure QApplication is created before the window

    window = MainWindow()  # Create the main window
    window.show()  # Show the PyQt5 window

    sys.exit(app.exec_())  # Start the event loop for PyQt5